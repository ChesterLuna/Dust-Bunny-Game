...
namespace SpringCleaning.Physics
{
    public class PatrolPlatform : PlatformBase
    {
        ...
        private float _currentTime, _currentPauseTime;
        ...
        protected override Vector2 Evaluate(float delta)
        {
            ...
            if (_ascending || _loop) _currentTime += delta / _duration;
            else _currentTime -= delta / _duration;

            _currentTime = Mathf.Clamp(_currentTime, 0, 1);
            var curveTime = _curve.Evaluate(_currentTime);

            if (_currentTime is >= 1 or <= 0)
            {
