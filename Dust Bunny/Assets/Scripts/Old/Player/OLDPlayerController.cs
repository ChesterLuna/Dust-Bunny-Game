...
public class OLDPlayerController : MonoBehaviour
{
    ...
    private void gatherInput()
    {
        ...
        if (UserInputOLD.instance.InteractInput)
        {
            sendInteract();
        }
        ...
        if (UserInputOLD.instance.JumpJustPressed)
        {
            _doJump = true;
        }

        if (UserInputOLD.instance.DashInput)
        {
            _doDash = true;
        }
    } // end gatherInput
    ...
    private void TurnCheck()
    {
        ...
        if (UserInputOLD.instance.MoveInput.x > 0 && !IsFacingRight)
        {
            Turn();
        }
        else if (UserInputOLD.instance.MoveInput.x < 0 && IsFacingRight)
        {
            Turn();
        }
    } // end TurnCheck
    ...
    void Move()
    {
        float _targetSpeed = UserInputOLD.instance.MoveInput.x * _moveSpeed;
    ...
    void updateFriction()
    {
        ...
        if (Mathf.Abs(UserInputOLD.instance.MoveInput.x) == 0f)
        {
    ...
    void Jump()
    {
        if (_currentDropDownPlatform != null && UserInputOLD.instance.MoveInput.y < 0f)
        {
    ...
    IEnumerator Dash()
    {
        ...
        _dashDirection = (UserInputOLD.instance.DashPositionInput - playerPos).normalized;
